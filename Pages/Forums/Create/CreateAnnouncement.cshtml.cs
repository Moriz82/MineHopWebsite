...
{
    public class CreateAnnouncement : PageModel
    {
        public void OnGet()
        {
        ...
        public void OnPostCreateReply(int sessionCount)
        {
            ...
            Forum fourm = new Forum(Forum.ForumType.Announcement, Title, username, Body);
            Program.Forums.Add(fourm);
            Program.UpdateForms();
            Program.SelctedForums.Remove(username);
            Program.SelctedForums.Add(username, fourm);
            HttpContext.Response.Redirect("/Forums/Announcement");
        }
    }
}