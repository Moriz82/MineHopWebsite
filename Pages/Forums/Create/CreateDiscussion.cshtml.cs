...
{
    public class CreateDiscussion : PageModel
    {
        public void OnGet()
        {
        ...
        public void OnPostCreateReply(int sessionCount)
        {
            ...
            Forum fourm = new Forum(Forum.ForumType.Discussion, Title, username, Body);
            Program.Forums.Add(fourm);
            Program.UpdateForms();
            Program.SelctedForums.Remove(username);
            Program.SelctedForums.Add(username, fourm);
            HttpContext.Response.Redirect("/Forums/Discussion");
        }
    }
}